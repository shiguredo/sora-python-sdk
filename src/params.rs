...
/// 劣化 preference を読む。組み立て器に受け口がないため検証のみ。
pub(crate) fn parse_degradation_preference(
    value: Option<Bound<'_, PyAny>>,
) -> PyResult<Option<i32>> {
    let Some(value) = value else {
        return Ok(None);
    ...
    let raw = value
        ...
        .map_err(|_| {
            PyValueError::new_err(
                "degradation_preference must be a SoraDegradationPreference or an int",
            )
        })?;
    if !(0..=3).contains(&raw) {
        return Err(PyValueError::new_err(format!(
            "degradation_preference must be within 0-3, got {raw}"
        )));
    }
    Ok(Some(raw as i32))
}
