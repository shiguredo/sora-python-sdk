...
class Platform(object):
    def _check(self, flag, error_message="Not supported"):
        if not flag:
    ...
    def _check_platform_target(self, p: PlatformTarget):
        if p.os == "raspberry-pi-os":
            ...
            self._check(p.arch is None,
                       f"Architecture should be None for {p.os}, but got {p.arch}")
        elif p.os == "ubuntu":
    ...
    def __init__(self, target_os, target_osver, target_arch, target_extra=None):
        ...
        if target.os == "ios":
            self._check(build.os == "macos",
                       f"iOS target requires macOS build platform, but got {build.os}")
            self._check(build.arch in ("x86_64", "arm64"),
        ...
        if target.os == "jetson":
            ...
            self._check(build.arch == "x86_64",
                       f"Jetson build requires x86_64, but got {build.arch}")
