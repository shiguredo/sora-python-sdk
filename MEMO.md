...
- Python からは `sora_sdk.__version__` でプロトタイプ自体の版を参照する。
- sora_sdk 側に公開の版取得 API はない (`src/version.rs` は `pub(crate)` のみ)。
  そのため依存版は `Cargo.lock` に記録する運用とする。

## モジュール名

- 拡張モジュール名は `sora_sdk` とした (置き換え時に昇格した)。
- 版は `VERSION` ファイルとそろえ、`2026.1.0` とする。
...
- `uv run maturin develop` で venv に導入し、`import sora_sdk` できる。
...
- C++ 実装 (`src/` の原始と CMake / setup 系) を削除し、
  Rust 実装を根元 (`src/`、`Cargo.toml`、`python/sora_sdk`、`tests/`) に移した。
- 旧 tests は名称と API 形状の互換要素で動かす。
  辞書 metadata、3 引数 Sink (直線補間の自前リサンプル)、
  列挙群、記録制御、符号化器情報、VAD (実力比の簡易判定と明記)、
  映像 encoded 変換、libcamera 受け口を追加した。
- 切断・切替の合成通知は暫定値を含む。
  `on_set_offer` は受信 offer 系記録から、`on_disconnect` は終了記録と
  WebSocket 終了と設定由来の定型文から、`on_switched` は要求値由来の
  暫定記録から合成する。処理器に受け口ができれば置き換える。
- 上流の受け口待ちで未対応のまま残す項目:
  `state` / `stream_id`、受信用 RPC 通知、音声 encoded 変換、
  単体 `forwarding_filter` 項目、音声 streaming 言語符号、
  劣化 preference の反映、送信側 simulcast rid、注視人数、
  路切替待ち上限。
- 動作の食い違いで未対応のまま残す項目:
  切替継続時の WebSocket 自己終了 (終了符号の即時性に影響)、
  同時配信用の符号化器表記 (`SimulcastEncoderAdapter` が付かない)。
- 旧 tests の選別結果:
  通らないのは上記の未対応項目を使うもの
  (`test_encoded_transform`、`test_degradation_preference`、
  切替継続の終了符号を求めるもの、同時配信の符号化器表記を求めるもの、
  単体転送選別器を求めるもの) と機器依存のもの
  (`test_raspberry_pi` 等の基盤選別) に限る。
  基盤選別で実行できないものは CI での確認が要る。
- CI の組み立て直し (maturin 化) は後続とする。
  C++ 用の組み立て workflow は削除し、E2E は wheel 取得経路のまま残した。
