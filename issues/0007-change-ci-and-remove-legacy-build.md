...
0001 〜 0006 で scikit-build-core 移行が完了した後、 CI を `uv build --wheel` ベースに完全統一し、 レガシービルドファイル（ `buildbase.py` / `run.py` / `pypath.py` / `MANIFEST.in` / `DEPS` ）を削除する。 あわせて PyPI publish 向け manylinux 化（ `auditwheel repair --strip --only-plat` ）と依存アーカイブの sha256 検証を導入し、 `CHANGES.md` の整合性を取る。

## 設計の前提（プロジェクト全体の新方針からの該当部）

- ビルド環境は ubuntu-24.04 x86_64 host のみ
- Linux arm64 は cross-compile only （ 0004 / 0005 ）。 arm64 native runner は **廃止** （ `build_ubuntu_arm` を完全削除）
- macOS / Windows はそれぞれ native build （ 0003 / 0006 ）
...
- レガシーファイル削除:
  - `buildbase.py`
  - `run.py` （ `format` サブコマンドの代替は 0008 で `Makefile` に移行済み前提）
  - `pypath.py`
  - `MANIFEST.in`
  - `DEPS` （内容は 0001 で `deps.json` に移行済み）
- `.github/workflows/build.yml` の全面整理:
  - `build_pyi` job を **完全削除** （ 0001 で `if: false` 済み）
  - `build_ubuntu_arm` job を **完全削除** （新方針で廃止。 0001 で `if: false` 済み）
  - `build_ubuntu` matrix から `ubuntu-22.04_x86_64` 系を再有効化するかどうかは「現状」セクション参照（ 0001 / 0004 / 0005 / 0006 完了時点で `ubuntu-24.04_x86_64` / `ubuntu-22.04_armv8` / `ubuntu-24.04_armv8` / `ubuntu-22.04_armv8_jetson` / `raspberry-pi-os_armv8` が動く前提）
  - 全 platform job で `uv run python run.py build ...` step が既に削除されている前提を確認し、残っていれば削除する
  - `e2e_test` job の `if: false` を解除し、 `./.github/workflows/e2e-test.yml` 側の hardcode artifact 名を 0001 〜 0006 で確定した artifact 名と整合させる
  - 各 platform job 末尾に manylinux 化ステップを追加:
    ...
    - macOS / Windows: manylinux 概念がないため `auditwheel` は呼ばない。 0001 / 0003 / 0006 のデフォルト wheel タグをそのまま使う
...
- 依存アーカイブの sha256 検証導入:
  ...
  - 0001 〜 0006 で取得したアーカイブの sha256 を実機で計算して `deps.json` に書き込む（ 0007 PR 内で 1 回実施）
- `CHANGES.md` の整理:
  - 既存 `## develop` の `[UPDATE] CMAKE_VERSION を 4.3.2 に上げる` エントリを削除（ 0001 で `[tool.scikit-build.cmake] version` 経由 PyPI 取得に切替済みのため）
...
- `auditwheel show` での実シンボル深検証（別 issue 。 0007 では `auditwheel repair --strip` が成功することのみ確認）
...
- `build_pyi` / `build_ubuntu_arm` / `build_macos` / `build_windows` / `e2e_test` の各 job:
  - `build_pyi`: 0001 で `if: false`
  - `build_ubuntu_arm`: 0001 で `if: false` （ 0007 で削除）
  - `build_macos`: 0003 で復活済み
  - `build_windows`: 0006 で復活済み
  - `e2e_test`: 0001 で `if: false`
- `build_ubuntu` matrix:
  - 0001: `ubuntu-24.04_x86_64` のみ
  - 0004: `ubuntu-22.04_armv8` / `ubuntu-24.04_armv8` 追加
  - 0005: `ubuntu-22.04_armv8_jetson` / `raspberry-pi-os_armv8` 追加
  - 0007: `ubuntu-22.04_x86_64` の扱いを確定（含めるか、 publish 対象から外すか）
- `setup.py` は 0001 で削除済み
- `run.py` / `buildbase.py` / `pypath.py` / `MANIFEST.in` / `DEPS` は残っているが scikit-build-core 経路では参照されていない
- `slack_notify` job の `needs:` は 0001 / 0003 / 0006 で `[build_ubuntu, build_macos, build_windows]` に確定済み
- `publish_wheel` / `create-release` は 0001 期間中タグを打たない運用で停止していた
...
2. `run.py` を `git rm` （ 0008 で `Makefile` に移行済みの `format` 機能を含めて完全削除）
3. `git grep -r "buildbase\|pypath\|MANIFEST\.in\|run\.py build"` で残存参照を確認
4. CHANGES.md にエントリ追加

`run.py` の `format` サブコマンドは 0008 で `make format` に移行する。 0007 着手前に 0008 が完了している前提（依存関係上は 0008 → 0007 ）。 もし順序を逆にする場合は `run.py format` を一時的に残して 0008 で削除する。
...
manylinux タグの正解値（実シンボル検証）は 0007 では行わない。 `auditwheel repair --only-plat` で必要な platform tag が自動的に決まる。 既存 `setup.py` で hardcode していたタグと一致しない場合は実体（ glibc バージョン）優先で確定する。
...
メインスクリプトで platform ごとに sha256 を `string(JSON GET)` で取り出して渡す。 sha256 が無い entry は検証 skip （既存 0001 〜 0006 移行期間との後方互換用）。
...
- `e2e_test.if: false` を削除し、 hardcode artifact 名を 0001 〜 0006 で確定した名前に合わせる
...
各 entry に sha256 フィールド追加。 0007 PR 内で実機 `curl -sL <url> | sha256sum` で計算する。
...
0007 マージ後に CI が大きく壊れた場合:

1. `git revert -m 1 <merge-commit>` で revert PR を作成
2. revert 後、 削除した legacy ファイルが復元されるか確認
3. `build_pyi` / `build_ubuntu_arm` job も復元されるが、 0001 〜 0006 で完了した scikit-build-core 経路と並存することになる
