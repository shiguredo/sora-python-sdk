...
- 製品コードのバグではなく、テスト側の期待値 (閾値) とビットレート立ち上がり待ちの設計の問題であり、メモリ破壊やクラッシュ等の実害は無い。
...
- VP8 / 240x128 では `simulcast_format_vp8` が `240*128 = 30720 <= 240*135 = 32400` の分岐に入り `150` を返す。これは 320x180 の target 値 (150 kbps) をそのまま流用した仮定値であり、240x135 は 320x180 よりピクセル数が約 44% 少なく補間領域 (target → 0 へ収束) にあるため、補間が有効な場合は過大推定になる。
- よって期待床 = `150 * 1000 * 0.3 = 45000`。
- 一方 WebRTC の VP8 サイマルキャストテーブル (`simulcast.py` の docstring に転記されている `kSimulcastFormatsVP8`) では、320x180 の target は 150 kbps、{0,0} の target は 0 kbps で、その間は線形補間される。240x135 のピクセル数 32400 は 320x180 の 57600 の約 56% であり、この補間が有効な場合の steady-state target は約 84 kbps と推定される。min の 30 kbps 床はキャップとして存在するが、84 > 30 なので通常は発動しない。
- ただし {0,0} への補間は WebRTC のフィールドトライアル `WebRTC-LowresSimulcastBitrateInterpolation` が有効な場合の挙動であり、無効の場合は低解像度層の target は最小定義エントリ (320x180 = 150 kbps) に張り付く。Sora SDK が使う WebRTC ビルド (`DEPS` の `WEBRTC_BUILD_VERSION=m150.7871.3.1`) でどちらの状態かを実装時に確認し、候補の選定根拠とすること。無効の場合、期待床 45000 は正当な値であり、失敗は純粋に timing 起因となる。
...
- 補間が有効なら「テストが仮定した期待床 45000」は「補間による steady-state 約 84000」の 30% (= 25200) を上回る過大値であり、閾値自体のミスキャリブレーションと timing の両方が flaky に寄与していることになる。
...
閾値側 (補間が有効な場合のミスキャリブレーション) と timing 側の両方に対処する。実装時に以下の候補から選定する (本 issue では断定しない):

1. 最小解像度層の `expect_target_bitrate` を WebRTC の補間テーブルの実態に整合させる。適用前にフィールドトライアル `WebRTC-LowresSimulcastBitrateInterpolation` が Sora SDK の WebRTC ビルドで有効か無効かを確認すること。無効なら 240x135 の steady-state target は 150 kbps のままであり、候補 1 は適用すべきでない。有効なら `simulcast_format_vp8` が 240x135 で 320x180 の target 値 (150) を流用している箇所を、補間による実効値 (約 84 kbps) に合わせる。ただし VP9 / AV1 は WebRTC テーブルに `{240, 135, target=101}` が明示的に定義されており、`simulcast_format_vp9` の返り値 101 はテーブルと一致している。VP9 / AV1 の問題は期待床 30300 と min 床 30000 のマージンが 300 bps (1%) しかないことであるため、候補 1 単独では VP9 / AV1 を解決できない。VP9 / AV1 には候補 3 (ratio 緩和) または候補 4 (ポーリング) の併用が必要。
...
- 修正の根拠 (WebRTC の補間テーブルとフィールドトライアルの状態、テストの期待値の関係) をコメント等で明確にすること。
