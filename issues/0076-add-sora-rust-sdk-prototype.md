...
- プロトタイプは `prototype/rust/` に隔離し、独自の `pyproject.toml` の maturin 経路と `Cargo.toml` を置く。ビルドは `uv run maturin build` で行い、現行の wheel ビルド経路に触れない。
- 着手時の第 1 段として sora-rust-sdk のリポジトリ URL と利用版を特定し、`prototype/rust/` 直下の `MEMO.md` に記録してから依存追加する。
...
- 調査結果と全面移行の後続作業の洗い出しは `prototype/rust/` 直下の `MEMO.md` に残す。

## 完了条件

- `prototype/rust/` でプロトタイプがビルドでき、Python から import できること。
...
- `prototype/rust/` 直下の `MEMO.md` に依存先 URL・版、API 対応メモ、`gil_used` 判定と理由、後続作業の洗い出しが残っていること。
