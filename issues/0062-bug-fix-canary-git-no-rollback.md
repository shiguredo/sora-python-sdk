# dev.py の git_operations が途中失敗時にローカルとリモートの整合が壊れる問題を修正する

- Priority: Medium
- Created: 2026-06-23
- Model: Opus 4.7
- Branch: feature/fix-dev-git-no-rollback
- Polished: 2026-08-28

## 目的

`dev.py` の `git_operations` は VERSION ファイルを更新後、`git add` → `git commit` → `git tag` → `git push` → `git push --tags` を `check=True` で 5 段直列に実行する。
...
この状態から開発者が何も知らずに `git pull` や次の `dev.py` を実行すると、ローカルのリリースコミット・リリースタグが不整合のまま伝搬する。
...
- dev.py は dev 版バンプ専用 (`[canary] Bump version to ...`) で頻繁に実行されるため、reject される実害は十分に発生し得る。
...
`dev.py` の `main()` は `update_version` → `run_uv_sync` → `git_operations` の順に呼び出す。`run_uv_sync` が `git add uv.lock` を実行済みであるため、`git_operations` の `git commit` は VERSION と uv.lock の両方をコミットする。

`dev.py` の `git_operations` の実装は次の通り。
...
- `dev.py` の `git_operations` が `git fetch` による事前検証を行い、fast-forward 不可能な場合はコミット・タグを作らずにエラー終了すること。
- タグ作成が `git push` 成功後に実行されること。
- 各ステップ失敗時に「何が残ったか」「どう復旧するか」のエラーメッセージが表示されること。
- 事前検証が fast-forward 不可能を検出する代表的なケース (リモートが先行している状態) の再現手順を PR 本文に記載すること。
- dry-run モードでは引き続き git コマンドを一切実行せず表示のみ行うこと (印字順は実経路の新順序に追従する)。
