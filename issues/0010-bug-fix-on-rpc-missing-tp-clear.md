...
本バグはリークが確率的ではなく決定論的に再現できるため、`0009` と違いリグレッションテストを書ける余地がある。

- `on_rpc` に接続オブジェクトを参照するハンドラを設定して参照循環を作り、外部からの強参照を消して `gc.collect()` を呼び、`weakref` で接続オブジェクトが回収されたかを検査する。修正前は回収されず、修正後は回収される、という差を観測する。
- ただし設計上の留意点が 2 つある。(a) 循環が接続オブジェクトの `tp_clear` を通してしか断ち切れない形で構成する必要がある (他の参加オブジェクト側の clear で断ち切れる循環だと修正前でも回収され、差が出ない)。(b) `SoraConnection` の生成にどこまで実 Sora サーバや有効な設定が必要かは要確認 (モック・スタブは使わない方針のため、生成に接続が要るなら実 Sora サーバが必要になる)。
- 上記が満たせない場合は、`0009` と同様に「コード検査による traverse/clear の対称性確認」を正当性の根拠とする。
