...
def test_receive_audio_and_video_with_new_api() -> None:
    ...
    def publish_video() -> None:
        video_result.update(
            sora_sdk.loopback_video_frames(
    ...
    def publish_audio() -> None:
        audio_result.update(
            sora_sdk.loopback_audio_frames(
    ...
    try:
        ...
        def on_track(track: sora_sdk.SoraMediaTrack) -> None:
            tracks.append(track)
            if track.kind == "audio":
                audio_sinks.append(sora_sdk.SoraAudioSink(track))
            elif track.kind == "video":

                def on_frame(frame: object) -> None:
                    video_frames.append(frame)

                sink = sora_sdk.SoraVideoSink(track)
        ...
        try:
            ...
            while time.time() < deadline:
                for sink in audio_sinks:
                    if not isinstance(sink, sora_sdk.SoraAudioSink):
                        continue
...
def test_audio_stream_sink_receives_frames() -> None:
    ...
    try:
        ...
        try:
            ...
            stream_sink = sora_sdk.SoraAudioStreamSink(audio_tracks[0])
