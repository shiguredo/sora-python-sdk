...
def test_intel_vpl_video_hwa_sendonly(setup, video_codec_type):
...
def test_intel_vpl_h264_sendonly_recvonly(setup):
    signaling_urls = setup.get("signaling_urls")
    channel_id_prefix = setup.get("channel_id_prefix")
    metadata = setup.get("metadata")
...
@pytest.mark.skip(reason="ローカルでは成功する")
def test_intel_vpl_h265_sendonly_recvonly(setup):
    signaling_urls = setup.get("signaling_urls")
    channel_id_prefix = setup.get("channel_id_prefix")
    metadata = setup.get("metadata")
