...
class SoraClient:
    def __init__(
        ...
        self._connection.on_push = self._on_push
    ...
    def _on_push(self, raw_message: str) -> None:
        self._push_queue.put(raw_message)

    def recv_push(self, timeout: float = 5) -> dict[str, Any]:
        return json.loads(self._push_queue.get(block=True, timeout=timeout))
    ...
    def recv_rpc(self, timeout: float = 5000) -> dict[str, Any]:
        return json.loads(self._rpc_queue.get(block=True, timeout=timeout).decode("utf-8"))

    def _on_rpc(self, data: bytes):
        print(f"Received RPC Response: data={data.decode('utf-8')}")
